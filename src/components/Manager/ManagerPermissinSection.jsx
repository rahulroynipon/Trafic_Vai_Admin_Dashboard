          ...
          {isLoading.profileGet ? (
            <ul className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-sm">
              {[...Array(6)].map((_, index) => (
                ...
                >
                  <div className="max-w-40 w-full h-4 bg-content-400/30 rounded animate-pulse" />
                </li>
              ))}
            </ul>
          ) : (
            <ul className="grid md:grid-cols-2 gap-x-4 gap-y-2.5 text-sm">
              {profile?.permissions?.map((permission, index) => (
                ...
                >
                  ...
                  <span className="text-content-300">{permission}</span>
                </li>
              ))}
            </ul>
          )}
